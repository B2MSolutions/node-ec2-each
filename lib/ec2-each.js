...
EC2.prototype.all = function(filters, callback) {
  this.ec2.DescribeInstances(filters, callback);
};

EC2.prototype.running = function(callback) {
  this.ec2.DescribeInstances({ FilterName  : [ 'instance-state-name'], FilterValue : [ [ 'running' ]] }, callback);
};

EC2.prototype.each = function(instances, action, callback) {
  _getTasks(action, instances, function(err, tasks) { 
    if(err) {
      return callback(err);
    }    
